...
#blocksize h2o:4081,co2:8000 14855
...
#     fout.flush()
#     total_para=0
#     print(f"{'name':<65} {'shape':<28} {'num_params':<12}")
#     for name, m in Prop_class.named_parameters():
#         total_para+=m.numel()
#         print(f"{name:<65} {str(m.shape):<28} {m.numel():<12,}")
#     print(f"total_para:{total_para}")

if dist.get_rank()==0:
    fout.write(time.strftime("%Y-%m-%d-%H_%M_%S \n", time.localtime()))
    fout.flush()
    print_model_summary(Prop_class, file=sys.stdout)
