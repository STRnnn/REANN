...
parser.add_argument(
    "--endingloss", default=0.001, type=float, help="Training end till reaching this given expected total loss"
)
...
def main():
    ...
    if args.hvd:
        train_sampler = torch.utils.data.distributed.DistributedSampler(
            train_dataset, num_replicas=hvd.size(), rank=hvd.rank(), drop_last=True
        )
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        if args.opt == "LKF" or args.opt == "GKF":
            ...
            loss, loss_Etot, loss_Force, loss_Ei, real_lr, epoch_time = train(
        ...
        is_best = (loss_Etot + loss_Force) < best_loss
        best_loss = min(loss_Etot + loss_Force, best_loss)
        ...
        if (epoch == args.epochs) or ((loss_Etot + loss_Force) <= args.endingloss) or (real_lr <= 3.0e-8):
            # evaluate on testing set
            vld_loss, vld_loss_Etot, vld_loss_Force, vld_loss_Ei, epoch_time = valid(
                val_loader, model, criterion, device, args
            )
            if not args.hvd or (args.hvd and hvd.rank() == 0):
                f_valid_log = open(valid_log, "a")
        ...
        if ((loss_Etot + loss_Force) <= args.endingloss) or (real_lr <= 3.0e-8):
            return
